      ...
      <div style={pageStyle}>
        ...
        {editor === 'services' && (
          <div style={card}>
            ...
            <div style={grid2}>
              <Input label="Nome servizio" value={serviceDraft.name} onChange={(value) => setServiceDraft((prev) => ({ ...prev, name: value }))} />
              <Input label="Categoria" value={serviceDraft.category} onChange={(value) => setServiceDraft((prev) => ({ ...prev, category: value }))} />
              <Input label="Durata (min)" type="number" value={serviceDraft.duration_minutes} onChange={(value) => setServiceDraft((prev) => ({ ...prev, duration_minutes: value }))} />
                  ...
                  {day.enabled && (
                    <div style={{ display: 'grid', gap: 8, marginTop: 10 }}>
                      {day.slots.map((slot, slotIndex) => (
                        <div key={`${day.weekday}-${slotIndex}`} style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                          <input type="time" value={slot.start_time} onChange={(event) => setAvailabilityDraft((prev) => prev.map((item, index) => index === dayIndex ? { ...item, slots: item.slots.map((currentSlot, currentIndex) => currentIndex === slotIndex ? { ...currentSlot, start_time: event.target.value } : currentSlot) } : item))} style={timeInput} />
                          <span style={{ color: '#6B7280' }}>→</span>
                          <input type="time" value={slot.end_time} onChange={(event) => setAvailabilityDraft((prev) => prev.map((item, index) => index === dayIndex ? { ...item, slots: item.slots.map((currentSlot, currentIndex) => currentIndex === slotIndex ? { ...currentSlot, end_time: event.target.value } : currentSlot) } : item))} style={timeInput} />
                        </div>
                      ))}
                    </div>
                  )}
